...
# Problem 1
def find_books_by_genre_and_language(*args):
    return Book.objects.filter(genre=args[0], language=args[1])
...
def find_authors_nationalities():
    ...
    for author in Author.objects.exclude(nationality__isnull=True):
        result.append(f"{author} is {author.nationality}")
...
def order_books_by_year():
    ...
    for book in Book.objects.all().order_by('publication_year', 'title'):
        result.append(
            f"{book.publication_year} year: {book}"
        )
...
def delete_review_by_id(review_id):
    ...
    return f"{review_to_delete} was deleted"
